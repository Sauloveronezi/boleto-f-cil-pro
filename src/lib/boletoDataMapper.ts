...
function aplicarTransformacao(row: Record<string, any>, mapeamento: MapeamentoCampo): string {
 ...
 */
function aplicarMapeamentoDinamico(row: Record<string, any>, mapeamentos: MapeamentoCampo[]): Record<string, string> {
...
export function mapBoletoApiToDadosBoleto(
  ...
  console.log(
    "[boletoDataMapper] banco_nome=",
    dados.banco_nome,
    "banco_codigo_formatado=",
    dados.banco_codigo_formatado,
    "banco_logo_url=",
    dados.banco_logo_url,
  );
  ...
  if (!dados.instrucoes) {
    ...
    const fmtCurrency = (v: number) =>
      v > 0 ? new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(v) : "R$ 0,00";
  ...
  switch (codigoBanco) {
    ...
    case "237": {
      // Bradesco: carteira/NNNNNNNNNNN-D
